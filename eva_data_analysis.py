...
def read_json_to_dataframe(input_file):
    ...
    # Find rows with non-numeric data in the 'eva' column
    # invalid_rows = eva_df[~eva_df['eva'].str.replace('.', '', 1).str.isnumeric()]
    # Remove unwanted characters
    eva_df['eva'] = eva_df['eva'].astype(str)
    eva_df['eva'] = eva_df['eva'].str.replace(r"[^\d.]", "", regex=True)
    eva_df['eva'] = pd.to_numeric(eva_df['eva'], errors='coerce')
    # Replace NaN values with 0
    eva_df['eva'] = eva_df['eva'].fillna(0)
